...
- **Global scope**: `D:/Programming/Repos/TaxReader/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Programming/Repos/TaxReader/.claude/notes/`)
...
2. Glob `D:/Programming/Repos/TaxReader/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/Programming/Repos/TaxReader/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Programming/Repos/TaxReader/.claude/notes/` — works in any directory
